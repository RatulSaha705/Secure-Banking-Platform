 ...
 *
 * POST /api/transfer/initiate        initiateTransferHandler (authenticated)
 * GET  /api/transfer/history         getHistoryHandler       (authenticated)
 * GET  /api/transfer/history/:txnId  getTransactionHandler   (authenticated)
 */

const { initiateTransfer, getMyTransactionHistory, getTransactionById } = require('../services/transferService');
...
const initiateTransferHandler = async (req, res, next) => {
  try {
    ...
    return res.status(201).json({ success: true, message: 'Transfer completed successfully.', data: receipt });
  } catch (err) {
...
const getTransactionHandler = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({ success: true, message: 'Transaction retrieved successfully.', data: txn });
  } catch (err) {
    if (err.statusCode) return sendError(res, err);
    return next(err);
  }
...
module.exports = { initiateTransferHandler, getHistoryHandler, getTransactionHandler };
